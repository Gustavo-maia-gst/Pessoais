...
class Assembler:
    ...
    def endfunc(self, funcname):
        self.code.append(f'add rsp, {8 * len(self.subroutines[funcname].locals)}')
    ...
    def startfunc(self, funcname):
        self.code.append('push rbp')
        self.code.append('mov rbp, rsp')

    def dofunc(self, funcname):
        if funcname in self.scanner.keywords:
        ...
        self.read_formal_params(funcname)

        self.startfunc(funcname)

        self.block(funcname)

        if funcname == 'main':
            self.code.append('call _exit')
            is_main = True
        else:
            is_main = False
            self.endfunc(funcname)
        
        self.subroutines[funcname].code = self.code
    ...
    def doprogram(self):
        ...
        while token and token.type == TokenType.IDENTIFIER:
            if self.dofunc(token.value): main_is_defined = True
            token = self.scanner.scan()

        if not main_is_defined:
    ...
    def compile(self):
        ...
        for variable in self.bss:
            code += f'\t{variable} resq 1\n'
            
        code += '\nsection .text\n'
        code += '\tglobal _start\n\n'

        for name, func in self.subroutines.items():
            code += f'{func.label}:\n'
            for line in self.subroutines[name].code: code += f'\t{line}\n'
            code += '\n'
